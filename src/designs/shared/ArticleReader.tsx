...
export function ArticleReader({ concept }: { concept: AstroConcept }) {
  ...
  return <article className="ar-reader" aria-labelledby={`${prefix}-title`}>
    ...
    {content.blocks.map((block, index) => {
      ...
      return variant ? <figure key={index}><img src={variant.src} alt={block.caption} loading="lazy" decoding="async" /><figcaption>{block.caption}</figcaption></figure> : null;
    })}
