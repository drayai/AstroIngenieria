const REVIEWED = 'revisado-validado';
const PENDING = 'pendiente';

const batches = [
  [0, 'R', REVIEWED, ['astroingenieria', 'kardashev', 'artificial-gravity', 'bernal-sphere', 'stanford-torus', 'bishop-ring', 'mckendree-cylinder', 'asteroid-habitat', 'worldship', 'iss', 'oneill-cylinder', 'life-support']],
  [1, 'A', REVIEWED, ['tipo-i', 'tipo-ii', 'tipo-iii']],
  [2, 'A', REVIEWED, ['civilizaciones-digitales', 'postbiological', 'deep-time']],
  [3, 'A', REVIEWED, ['von-neumann', 'colonizacion-galactica']],
  [3, 'B', REVIEWED, ['grabby-aliens']],
  [4, 'A', PENDING, ['great-filter']],
  [4, 'B', PENDING, ['zoo-hypothesis', 'dark-forest', 'civilizaciones-silenciosas', 'berserker']],
  [5, 'A', PENDING, ['seti']],
  [5, 'B', PENDING, ['technosignatures', 'radio-seti', 'optical-seti', 'civilizaciones-y-luz']],
  [6, 'A', PENDING, ['project-orion', 'fusion-propulsion', 'antimatter']],
  [6, 'B', PENDING, ['bussard-ramjet']],
  [7, 'A', PENDING, ['alcubierre', 'wormholes']],
  [7, 'B', PENDING, ['reactionless']],
  [8, 'B', PENDING, ['ion-engines', 'hall-thruster', 'solar-electric', 'nuclear-electric']],
  [9, 'B', PENDING, ['chemical-rockets', 'nuclear-thermal', 'solar-sail', 'beamed-propulsion']],
  [10, 'A', PENDING, ['laser-sail']],
  [10, 'B', PENDING, ['magnetic-sail', 'electric-sail', 'interstellar-braking']],
  [11, 'A', PENDING, ['venus-terraforming', 'floating-venus']],
  [12, 'B', PENDING, ['paraterraforming', 'worldhouse', 'domed-cities']],
  [13, 'A', PENDING, ['habitability', 'ecopoiesis']],
  [13, 'B', PENDING, ['habitable-zone', 'planetary-protection']],
  [14, 'B', PENDING, ['orbital-mirrors', 'sunshades', 'volatile-import', 'magnetosphere']],
  [15, 'A', PENDING, ['stellar-engines', 'caplan']],
  [15, 'B', PENDING, ['shkadov']],
  [16, 'A', PENDING, ['star-lifting']],
  [16, 'B', PENDING, ['stellar-husbandry', 'plasma-processing']],
  [17, 'B', PENDING, ['stellar-physics', 'stellar-navigation', 'stellar-technosignatures']],
  [18, 'A', PENDING, ['orbital-ring', 'launch-loop']],
  [18, 'B', PENDING, ['skyhook', 'tethers', 'mass-driver']],
  [19, 'B', PENDING, ['shipyards', 'orbital-ports', 'fuel-depots', 'reusable-launch']],
  [20, 'A', PENDING, ['lunar-bases']],
  [20, 'B', PENDING, ['isru', 'asteroid-mining']],
  [21, 'B', PENDING, ['space-based-solar', 'microwave-power', 'radiators', 'dyson-ring', 'dyson-bubble']],
  [22, 'A', PENDING, ['matrioshka-brain']],
  [22, 'B', PENDING, ['jupiter-brain', 'computronium', 'dyson-shell']],
  [23, 'A', PENDING, ['exoplanets', 'future-universe', 'ringworld']],
  [24, 'B', PENDING, ['space-law', 'cosmic-ethics']],
  [25, 'R', PENDING, ['space-elevator', 'dyson-swarm', 'relativistic-propulsion', 'terraforming', 'mars-terraforming', 'black-hole-engineering', 'fermi', 'astrobiology']],
];

export const editorialMinimums = { A: 900, B: 650, R: 0 };
export const editorialStatuses = ['pendiente', 'en-redaccion', 'redactado-pendiente-revision', REVIEWED];

export const editorialPlan = {};
for (const [batch, category, status, ids] of batches) {
  for (const id of ids) {
    if (editorialPlan[id]) throw new Error(`Duplicate editorial plan id: ${id}`);
    editorialPlan[id] = { batch, category, status };
  }
}
